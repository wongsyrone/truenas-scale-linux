...
static const struct smb_sid sid_everyone = {
	1, 1, {0, 0, 0, 0, 0, 1}, {0} };
#ifdef CONFIG_TRUENAS
static const struct smb_sid sid_creator_owner = {
	1, 1, {0, 0, 0, 0, 0, 3}, {0} };

static const struct smb_sid sid_creator_group = {
...
get_account_special_sid_type(struct smb_sid *psid)
{
 ...
 */
static int
convert_smb_sid_to_nfs_who_special(struct smb_sid *psid,
				   u32 *iflag,
				   u32 *who_id,
				   u32 *flags)
{
...
static int
convert_smb_sid_to_nfs_who(struct smb_sid *psid, u32 *iflag, u32 *who_id, u32 *flags)
{
	char *sidstr;
	const struct cred *saved_cred;
	struct key *sidkey;
...
static int
do_ace_conversion(struct smb_ace *pace,
		  u32 *p_perms,
		  u32 *p_iflag,
		  u32 *p_who_id,
		  u32 *p_flags,
...
static bool
combine_with_next(struct smb_ace *pace,
		  u32 *p_perms,
		  u32 *p_iflag,
		  u32 *p_who_id,
		  u32 *p_flags,
 ...
 */
static int
convert_smbace_to_nfsace(struct smb_ace *pace,
			 u32 *zfsacl,
			 bool isdir,
			 uid_t owner,
			 uid_t group,
...
static int
convert_dacl_to_zfsacl(struct smb_acl *dacl_ptr,
		       char *end,
		       struct inode *inode,
		       char **buf_out)
{
...
int ntsd_to_zfsacl_xattr(struct smb_ntsd *pntsd,
			 u32 acl_len,
			 struct inode *inode,
			 char **buf_out)
{
...
static int
convert_zfsperm_to_ntperm(u32 zfsperms, struct smb_ace *ace)
{
	u32 access_mask = 0;
	int i;
...
static int
convert_zfsflag_to_ntflag(u32 zfsflags, struct smb_ace *ace)
{
	u8 flags = 0;
	int i;
...
static int
convert_zfstype_to_nttype(u32 ace_type, struct smb_ace *ace)
{
	switch (ace_type) {
...
static int
convert_zfswho_to_ntsid(u32 iflag, u32 who_id, struct inode *inode, u32 flags, struct smb_ace *ace)
{
...
#define BASE_ACE_SIZE (1 + 1 + 2 + 4) /* struct smb_ace: type, flags, size, access_req */
...
convert_zfsace_to_cifs_aces(u32 *zfsace, char *acl_base, struct inode *inode, u16 *size)
{
	...
	int error;
	u16 out_sz = 0, ace_sz;
	struct smb_ace *ace = (struct smb_ace *)acl_base;
	...
	if ((iflag & ACEI4_SPECIAL_WHO) && (who_id != ACE4_SPECIAL_EVERYONE) &&
		...
		ace = (struct smb_ace *)(acl_base + ace_sz);
...
convert_zfsacl_to_cifsacl(u32 *aclbuf,
			  ...
			  u16 *pacl_size_out)
{
	u32 i, nsize = sizeof(struct smb_acl);
 ...
 */
static int
parse_single_ace(u32 *zfsace, struct smb_ntsd **ppntsd_out, u32 *acllen_out)
{
	u32 perms, flags, iflag, who_id, ace_type;
	bool dacl_is_null = false, dacl_is_empty = false;
	u32 secdesclen = sizeof(struct smb_ntsd);
	struct smb_ntsd *pnntsd = NULL;
	struct smb_acl *dacl = NULL;
	...
	*acllen_out = secdesclen;
	...
	/* dacl_is_empty */
	pnntsd->dacloffset = cpu_to_le32(sizeof(struct smb_ntsd));
	dacl = (struct smb_acl *)(pnntsd + sizeof(struct smb_ntsd));
			 ...
			 u32 *acllen_out)
{
	...
	secdesclen = dacl_ace_cnt * sizeof(struct smb_ace);
	...
	dacl = (struct smb_acl *)((char*)pnntsd + pnntsd->dacloffset);
